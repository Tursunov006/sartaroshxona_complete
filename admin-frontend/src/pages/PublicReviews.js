              ...
              <Form onSubmit={handleSubmit}>
                ...
                <Form.Group className="mb-3">
                  ...
                  <Form.Select
                    className="form-select-modern"
                    value={form.shopId}
                    onChange={e => setForm({ ...form, shopId: e.target.value, barberId: '' })}
                  >
                    <option value="">— Salon tanlang —</option>
                    {shops.map(shop => (
                      <option key={shop.id || shop._id} value={shop.id || shop._id}>{shop.name}</option>
                    ))}
                  </Form.Select>
                </Form.Group>
                <Form.Group className="mb-3">
                  ...
                  <Form.Select
                    className="form-select-modern"
                    value={form.barberId}
                    onChange={e => setForm({ ...form, barberId: e.target.value, shopId: '' })}
                  >
                    <option value="">— Sartarosh tanlang —</option>
                    {barbers.map(barber => (
                      <option key={barber.id || barber._id} value={barber.id || barber._id}>{barber.name}</option>
                    ))}
                  </Form.Select>
                </Form.Group>
        ...
        <Col lg={7}>
          {loading ? (
            ...
            <div className="d-grid gap-3">
              ...
              {reviews.map(review => (
                <Card key={review.id || review._id} className="feature-card card-modern h-100 p-4 review-card">
                  <div className="d-flex justify-content-between align-items-center mb-3">
                    <div>
                      <strong>{review.userName || 'Mehmon'}</strong>
                      <div className="small-note">{getTargetName(review)}</div>
                    </div>
